package com.bignerdranch.android.geoquizv5_test_upload2

import android.util.Log
import androidx.lifecycle.ViewModel
import java.lang.reflect.Array.get

private const val TAG = "QuizViewModel"

class QuizViewModel: ViewModel() {

    /*private*/ val questionBank = listOf(
        Question(R.string.question_asia,true, 0),
        Question(R.string.question_oceans,true, 0),
        Question(R.string.question_mideast,false, 0),
        Question(R.string.question_africa,true, 0),
        Question(R.string.question_americas,false, 0),
        Question(R.string.question_australia,false, 0)
    )

    var currentIndex = 0
    var numAnswered = 0
    var numCorrectAnswered = 0

    val currentQuestionAnswer: Boolean
        get() = questionBank[currentIndex].answer

    val currentQuestionText: Int
        get() = questionBank[currentIndex].textResId

    fun moveToNext(){
        currentIndex = (currentIndex + 1) % questionBank.size
    }

}